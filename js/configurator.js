$(function() {
  ...
  // Bootstrap 5 modal instances (Bootstrap no longer provides jQuery plugins)
  const vendorPickerEl = document.getElementById('vendorPicker');
  const configuratorEl = document.getElementById('configuratorMain');
  const vendorPickerModal = bootstrap.Modal.getOrCreateInstance(vendorPickerEl);
  const configuratorModal = bootstrap.Modal.getOrCreateInstance(configuratorEl);

  // HTML5 validation form element
  ...
  function initApp() {
    ...
    // Bootstrap 5 does not toggle .active for "button" radios automatically with this markup.
    // Keep your existing label/input markup and implement a small toggler:
    $('#configuratorMain').on('click', '.btn-group label.btn', function() {
      var $label = $(this);
    ...
    // HTML5 validation + bootstrap styling
    configFormEl.addEventListener('submit', function(e) {
      e.preventDefault();
      e.stopPropagation();

      if (!configFormEl.checkValidity()) {
        configFormEl.classList.add('was-validated');
        return;
      }

      // if valid
      configFormEl.classList.remove('was-validated');
      addUser();
    });

    $('#addNewDisk').on('click', function() {
      // trigger submit so validation runs
      if (configFormEl.requestSubmit) {
        configFormEl.requestSubmit();
      } else {
        $(configFormEl).trigger('submit');
      }
    });
    ...
    $('#btnRestartApp').on('click', function() {
      resetPage();
    });

    $('table#vendors img').click(function() {
      ...
      // reset validation UI when opening
      configFormEl.classList.remove('was-validated');
    });

    configuratorEl.addEventListener('hidden.bs.modal', function() {
      resetConfigurator();
    });

    // Use input change handlers (more reliable than label change)
    $('#gen input[type=radio]').on('change', function() {
      ...
      if (gen !== '5/6/7') {
        ...
        if (vendorType !== 'dell' && vendorType !== 'sm' && vendorType !== 'emc') {
          ['SASMDL', 'SATAMDL', 'FC', 'SSNW'].forEach(function(s) {
    ...
    $('#diskType input[type=radio]').on('change', function() {
  ...
  function addUser(e, formFactor, interfaceType, interfaceSpeed, diskSpeed, diskType, diskCapacity, diskSerial, targetTable, targetNumber, gen) {
    ...
    if (gen !== 'gen8') {
      switch (interfaceSpeed) {
        case 'SATA2/SAS':
          if (/SATA/.test(diskType)) { diskSpeedLabel = diskSpeedLabel.concat('3G '); }
          break;
        case 'SATA3/SAS2':
      ...
      case 'sm':
        ...
        switch (formFactor) {
          case 'SFF':
            ...
            break;
          case 'LFF':
            labelString = "<div class='sm LFF'><div class='interface'>{0} \n {1}</div></div>".format(diskType, diskCapacity);
            break;
        }
        break;
    ...
    if (($('#' + targetTable + ' tr').length === 0) || ($('#' + targetTable + ' tr:last td').length >= targetNumber)) {
      $('#'+targetTable+' tbody').append('<tr>');
    }
  ...
  function isSelectorOptionDisabled(selector, label) {
    return selector.parent().find('label[for=\'' + label + '\']').hasClass('disabled');
  }
